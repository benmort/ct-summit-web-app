...
export default function ShowreelCarousel({ photos, loading, brokenIds }: Props) {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col justify-center overflow-hidden bg-scrim">
      <div
      ...
      {loading && (
        <p className="absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 text-sm text-on-scrim-muted">
          Loading showreel…
        </p>
      )}
            ...
            {slides.map((photo, i) => (
              ...
              >
                ...
                <img
                  src={photo.url}
                  alt=""
                  className="pointer-events-none block h-full w-auto max-w-none select-none rounded-2xl ring-1 ring-veil/10"
                  draggable={false}
                  decoding="async"
                />
              </div>
